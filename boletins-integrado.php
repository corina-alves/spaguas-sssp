...
$total_query = $conn->query("SELECT COUNT(*) as total FROM boletimIntegrado2 $where");
$total_result = $total_query->fetch_assoc();
$total = $total_result['total'];
$total_paginas = ceil($total / $por_pagina);

// Consulta paginada
$sql = "SELECT * FROM boletimIntegrado2 $where ORDER BY data_upload DESC LIMIT $inicio, $por_pagina";
...
<head>
  <meta charset="UTF-8">
  <title>Boletins Boletim sobre Situação Hídrica</title>
 ...
 <?php include "menuspgov.php";?>
  <header id="header" class="header  sticky-top"><?php include "nav.php";?></header>
       ...
       <div class="col-lg-6 offset-lg-6" data-aos="fade-right">
          <h1><strong>Boletim sobre Situação Hídrica</strong></h1>
          <p>Boletim sobre a situação hídrica, com informações sobre a situação hídrica do Sistema Integrado Metropolitano (SIM). Este boletim é uma publicação conjunta da ARSESP e da SP-ÁGUAS, elaborado no âmbito do Comitê de
            Integração das Agências para a Segurança Hídrica, criado pela Portaria Conjunta nº 01/2025. Seu objetivo
            é divulgar informações atualizadas sobre a situação hidrológica e operação do abastecimento urbano na
            Região Metropolitana de São Paulo (RMSP), como apoio à tomada de decisão regulatória, visando garantir
            a comunicação clara e transparente à sociedade.</p>
        </div>
  </div></div>
</section>
...
<div class="div-scroll">
    ...
    <form method="GET" class="mb-4 d-flex">
        ...
        <a href="boletins-integrado.php" type="button" class="btn btn-secundary">Limpar</a>
    </form>
        ...
        <tbody>
            <?php if ($result->num_rows > 0): ?>
                ...
                <tr>
                    ...
                    <td><a href="upBoletimIntegrado/<?= $row['arquivo'] ?>" target="_blank">Visualizar</a></td>
