...
function get_dados_chuva_data(DateTime $data) {
    $cacheDir = __DIR__ . '/cache_chuva';
    if (!file_exists($cacheDir)) mkdir($cacheDir, 0777, true);

    $cacheFile = $cacheDir . '/chuva_' . $data->format('Y-m-d') . '.json';
    $tempo_cache = 3600; // 1 hora (em segundos)

    // Se o cache existe e é recente, usa ele
    if (file_exists($cacheFile) && (time() - filemtime($cacheFile) < $tempo_cache)) {
...
<head>
<meta charset="utf-8" />
<title>Precipitação dos Sistemas Produtores</title>
<link href="assets/img/logo/logo.png" rel="icon">
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-sRIl4kxILFvY47J16cr9ZwB07vP4J8+LH7qKQnuqkuIAvNWLzeN8tE5YBujZqJLB" crossorigin="anonymous">
<link href="assets/css/aptos.css" rel="stylesheet">
...
<h2 style="text-align:center; margin-top:30px;">Acompanhamento de <strong>Chuva </strong>nos Sistemas Produtores (mm)</h2>
        <h5 style="text-align:center; margin-bottom:30px;">
            Dados de <?= $data_base->format('d/m/Y') ?> | Ano de referência: <?= $ano_ref ?>
        </h5>
<!-- Tabela -->
...
<div class="row justify-content-center my-3">
  <div class="col-md-6">
    <form method="GET" class="d-flex align-items-center gap-2 justify-content-center">
      ...
      <input type="date" id="date" name="date" class="form-control" value="<?= $data_base->format('Y-m-d') ?>"  max="<?= date('Y-m-d') ?>"
      >
...
<table class="table table-striped table-bordered">
    <thead>
        <!-- <tr>
  <th colspan="9" class="text-center bg-success text-white">Acompnhamento da Chuva </th>
</tr>
        <tr> -->
            <?php foreach (array_keys($linhas[0]) as $h): ?>
                <th class="bg-primary text-white"><?= htmlspecialchars($h) ?></th>
            <?php endforeach; ?>
        </tr>
    </thead>
    <tbody>
...
new Chart(ctx, {
    ...
    options: {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: 'Comparativo de Chuvas por Sistema (mm)',
            ...
            datalabels: {
                ...
                formatter: (value) => value !== null && !isNaN(value) ? value.toFixed(1) + '' : ''
            }
        }
    },
    plugins: [ChartDataLabels] // ✅ Ativa o plugin no gráfico
});
</script>
